...
function sendOTPRequest(name, mobile) {
  ...
  return fetch('/guru-mantra/auth/send_otp', {
    method: 'POST',
    headers: headers,
    body: JSON.stringify({
      name: name,
...
function verifyOTPRequest(otp) {
  ...
  return fetch('/guru-mantra/auth/verify_otp', {
    method: 'POST',
    headers: headers,
    body: JSON.stringify({
      otp: otp
...
function handleOTPSuccess(mobile, messageOrResponse) {
  ...
  let message = '';
  let redirectUrl = '/guru-mantra';  // default redirect
...
// Log initialization
console.log('Guru Mantra Sadhana Login initialized successfully');
